defmodule JournalexWeb.Analytics.TickersLive do
  ...
  def handle_event("search", %{"q" => q}, socket) do
    {:noreply, assign(socket, search_q: q, filtered_tickers: filter_tickers(socket.assigns.tickers, q))}
  end
  ...
  defp reload(socket, changes) do
    ...
    tickers = Analytics.ticker_summary(opts)
    filtered = filter_tickers(tickers, a.search_q)
    assign(socket, tickers: tickers, filtered_tickers: filtered)
  end

  defp filter_tickers(tickers, ""), do: tickers

  defp filter_tickers(tickers, q) do
    q_down = String.downcase(q)
    Enum.filter(tickers, fn {ticker, _, _, _, _, _} ->
      String.contains?(String.downcase(ticker), q_down)
    end)
  end
