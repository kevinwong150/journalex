defmodule JournalexWeb.TradesDatesLive do
  ...
  def mount(_params, _session, socket) do
    ...
    results =
      case Trades.list_trades_between(sd, ed) do
        {:error, _} -> []
          ...
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
              ...
              <button
                ...
                aria-expanded={@trades_expanded}
                aria-controls="trades-table"
              >
                <%= if @trades_expanded do %>
                  Collapse
                <% else %>
                  Expand
                <% end %>
  ...
  def handle_params(params, _uri, socket) do
    ...
    cond do
      is_binary(start_date) and is_binary(end_date) and String.length(start_date) == 8 and
        ...
        case Trades.list_trades_between(start_date, end_date) do
          {:error, _} ->
            {:noreply,
             assign(socket,
          ...
          results when is_list(results) ->
            # Rebuild date_grid with ALL trades to show accurate tick marks
            all_trades_for_month = load_trades_for_calendar_month(socket.assigns.calendar_month)

            date_grid =
              build_date_grid_for_month(socket.assigns.calendar_month, all_trades_for_month)

            # Prepare summary data
            {summary_by_symbol, summary_total} = summarize_trades(results)

            # Calculate distinct trading days across ALL trades in the selected range
            distinct_days = calculate_distinct_days(results)
  ...
  def handle_event("submit_dates", %{"dates" => %{"start_date" => sd, "end_date" => ed}}, socket) do
    ...
    case valid_range?(sd, ed) do
      true ->
        {:noreply, push_patch(socket, to: ~p"/trade/dates?start_date=#{sd}&end_date=#{ed}")}
  ...
  defp build_date_grid(sd, ed, trades) when is_binary(sd) and is_binary(ed) do
    with {:ok, d1} <- to_date(sd), {:ok, d2} <- to_date(ed) do
      present =
        trades
        |> Enum.map(fn t -> t.datetime |> DateTime.to_date() end)
        |> MapSet.new()
  ...
  # Load all trades for a specific calendar month (for accurate tick marks)
  defp load_trades_for_calendar_month(%Date{} = first) do
    last = end_of_month(first)
    sd = yyyymmdd(first)
    ed = yyyymmdd(last)
  ...
  # Build date grid for a single month with all trades for that month
  defp build_date_grid_for_month(%Date{} = first, trades) do
    last = end_of_month(first)
    sd = yyyymmdd(first)
    ed = yyyymmdd(last)
    build_date_grid(sd, ed, trades)
  end
  ...
  # Summarize trades by ticker for ActivityStatementSummary component
  defp summarize_trades(trades) do
    groups = Enum.group_by(trades, & &1.ticker)

    rows =
      groups
      |> Enum.map(fn {ticker, ticker_trades} ->
        total_pl = ticker_trades |> Enum.map(&to_number(&1.realized_pl)) |> Enum.sum()

        wins = Enum.count(ticker_trades, fn t -> to_number(t.realized_pl) > 0.0 end)
        total_count = length(ticker_trades)

        days_traded =
          ticker_trades
          |> Enum.map(fn t -> Date.to_iso8601(DateTime.to_date(t.datetime)) end)
          |> Enum.uniq()
          |> length()
        ...
        %{
          ...
          close_count: total_count,
          close_positive_count: wins,
          days_traded: days_traded,
          close_trades: ticker_trades
        }
      end)
  ...
  # Calculate the number of distinct trading days across all trades
  defp calculate_distinct_days(trades) do
    trades
    |> Enum.map(fn t -> Date.to_iso8601(DateTime.to_date(t.datetime)) end)
    |> Enum.uniq()
    |> length()
