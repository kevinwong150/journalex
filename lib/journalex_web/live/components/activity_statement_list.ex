defmodule JournalexWeb.ActivityStatementList do
			...
			<div class="px-6 py-4 border-b border-gray-200">
				<div class="flex items-center justify-between gap-3">
					<div class="flex items-center gap-3">
						...
						<%= if @toggle_event do %>
							<button
								...
								aria-controls={@id}
							>
								<%= if @expanded do %>Collapse<% else %>Expand<% end %>
							</button>
						<% end %>
					</div>
					...
					<div :if={@show_save_controls?} class="flex items-center gap-2">
						...
						<.link :if={@on_upload_path?} navigate={@on_upload_path?}
			...
			<div class={"overflow-x-auto #{unless @expanded, do: "hidden"}"} id={@id}>
				<table class="min-w-full divide-y divide-gray-200">
					<thead class="bg-gray-50">
						<tr>
							<th :if={@show_save_controls?} class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Save</th>
							<th :if={@show_save_controls?} class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Status</th>
					...
					<tbody class="bg-white divide-y divide-gray-200">
						<%= for {row, idx} <- Enum.with_index(@rows) do %>
							<tr class="hover:bg-gray-50">
								...
								<td class="px-6 py-4 whitespace-nowrap text-sm text-right text-gray-900">{if @show_values?, do: display_number(Map.get(row, :quantity)), else: ""}</td>
						...
						<%= if Enum.empty?(@rows) do %>
							<tr>
								<td colspan={if @show_save_controls?, do: 13, else: 11} class="px-6 py-8 text-center text-sm text-gray-500">
								</td>
							</tr>
						<% end %>
					</tbody>
	...
	defp pl_class(<<"-", _rest::binary>>), do: "text-red-600"
		defp pl_class(%Decimal{} = d) do
			cond do
				Decimal.cmp(d, 0) == :lt -> "text-red-600"
				Decimal.cmp(d, 0) == :gt -> "text-green-600"
				true -> "text-gray-900"
			end
		end
